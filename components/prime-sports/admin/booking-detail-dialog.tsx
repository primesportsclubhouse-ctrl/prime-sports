'use client';

import { X } from "lucide-react";
import { useEffect } from "react";

import {
  CalendarBooking,
  primeButtonOutlineClass,
  primeMetaLabelClass,
  primeSectionTitleClass,
  primeStatusPillBaseClass,
  primeToolbarIconButtonClass,
} from "@/lib/prime-sports";

type BookingDetailDialogProps = {
  booking: CalendarBooking;
  sportLabel: string;
  courtLabel: string;
  date: string;
  time: string;
  onClose: () => void;
};

export default function BookingDetailDialog({
  booking,
  sportLabel,
  courtLabel,
  date,
  time,
  onClose,
}: BookingDetailDialogProps) {
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      if (event.key === "Escape") {
        onClose();
      }
    }

    window.addEventListener("keydown", handleKeyDown);
    return () => window.removeEventListener("keydown", handleKeyDown);
  }, [onClose]);

  return (
    <div
      className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="booking-detail-title"
        className="w-full max-w-[520px] rounded-[var(--radius)] border border-border bg-surface text-foreground shadow-[var(--shadow-md)]"
        onClick={(event) => event.stopPropagation()}
      >
        <div className="flex items-start justify-between gap-4 border-b border-border p-6 pb-4">
          <div>
            <p className="mb-1.5 text-xs font-bold uppercase tracking-[0.14em] text-accent">Booking Details</p>
            <h3 id="booking-detail-title" className={primeSectionTitleClass}>
              {booking.name}
            </h3>
          </div>
          <button type="button" aria-label="Close" className={primeToolbarIconButtonClass} onClick={onClose}>
            <X size={16} aria-hidden="true" />
          </button>
        </div>

        <div className="p-6">
          <span
            className={`${primeStatusPillBaseClass} ${
              booking.pending
                ? "border-accent bg-[rgba(200,55,45,0.12)] text-accent"
                : "border-accent-secondary bg-[rgba(212,163,89,0.12)] text-accent-secondary"
            }`}
          >
            {booking.pending ? "Pending" : "Confirmed"}
          </span>

          <dl className="mt-5 grid grid-cols-2 gap-x-4 gap-y-4 max-[480px]:grid-cols-1">
            <div>
              <dt className={primeMetaLabelClass}>Sport</dt>
              <dd className="m-0 text-[13px] font-semibold">{sportLabel}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Court</dt>
              <dd className="m-0 text-[13px] font-semibold">{courtLabel}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Date</dt>
              <dd className="m-0 text-[13px] [font-family:var(--font-mono)] font-semibold tabular-nums">{date}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Time Slot</dt>
              <dd className="m-0 text-[13px] [font-family:var(--font-mono)] font-semibold tabular-nums">{time}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Amount</dt>
              <dd className="m-0 text-[13px] [font-family:var(--font-mono)] font-semibold tabular-nums">{booking.amount}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Channel</dt>
              <dd className="m-0 text-[13px] font-semibold">{booking.channel}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Reference</dt>
              <dd className="m-0 text-[13px] [font-family:var(--font-mono)] font-semibold tabular-nums">{booking.ref}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Submitted</dt>
              <dd className="m-0 text-[13px] opacity-70">{booking.submitted}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Phone</dt>
              <dd className="m-0 text-[13px] [font-family:var(--font-mono)] font-semibold tabular-nums">{booking.phone}</dd>
            </div>
            <div>
              <dt className={primeMetaLabelClass}>Email</dt>
              <dd className="m-0 text-[13px] [font-family:var(--font-mono)] font-semibold tabular-nums">{booking.email}</dd>
            </div>
          </dl>
        </div>

        <div className="flex justify-end border-t border-border p-6 pt-5">
          <button type="button" className={primeButtonOutlineClass} onClick={onClose}>
            Close
          </button>
        </div>
      </div>
    </div>
  );
}
