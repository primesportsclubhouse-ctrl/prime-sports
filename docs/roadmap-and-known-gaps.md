...
- **Booking emails** (Resend) — two distinct, verified sends: a "booking received, pending verification" email the moment a guest submits a payment reference, and a separate "booking confirmed" email once staff approve it. Configured with a verified sending domain, and always sent with a real "PrimeSports Clubhouse" sender display name (not the bare address, which inboxes like Gmail otherwise render as an unprofessional-looking raw local-part).
...
- **Master Calendar** (`/admin/dashboard`) — real `bookings` → `customers` → `payment_submissions` join (`GET /api/admin-calendar`), scoped to `pending_payment`/`confirmed` bookings only, with realtime refresh. Replaced the old deterministic fake grid (`createAdminBookings()`, fully removed) entirely. The detail dialog's old "Customer Notes" field was also removed — it was structurally guaranteed to always read empty (`payment_submissions.notes` is only ever populated by staff on reject, and a rejected submission cancels its booking, so it can never appear in this pending/confirmed-only view).
- **Optimistic booking selection** — picking a slot on `/reserve/schedule` now reflects immediately (checkmark, "Selected" styling) instead of waiting on the `create_booking_draft()` round-trip, rolling back only if the request genuinely fails (e.g. a real double-booking race). Removing a slot also immediately clears its live-availability status locally, instead of flashing "Held"/"Booked" until the next realtime-triggered refetch catches up.
