...
- **Booking emails** (Resend) — two distinct, verified sends: a "booking received, pending verification" email the moment a guest submits a payment reference, and a separate "booking confirmed" email once staff approve it. Configured with a verified sending domain.
...
- **Master Calendar** (`/admin/dashboard`) — real `bookings` → `customers` → `payment_submissions` join (`GET /api/admin-calendar`), scoped to `pending_payment`/`confirmed` bookings only, with realtime refresh. Replaced the old deterministic fake grid (`createAdminBookings()`, fully removed) entirely.
